/**
 * AI Chat Bridge — HTTP Bridge (Content Script)
 * 
 * Polls the HTTP bridge server for incoming requests and forwards them
 * to the background service worker for AI interaction.
 */

let BRIDGE_URL = 'http://localhost:8765';
let pollInterval = null;
const POLL_DELAY = 500;
let requestsInFlight = 0;

// Load bridge URL from config
async function loadBridgeUrl() {
  return new Promise(resolve => {
    chrome.storage.sync.get('aiBridgeConfig', (result) => {
      const config = result.aiBridgeConfig || {};
      BRIDGE_URL = config.bridgeUrl || 'http://localhost:8765';
      resolve(BRIDGE_URL);
    });
  });
}
...
loadBridgeUrl().then(() => {
  connect();
  log('HTTP Bridge initialized — bridge URL:', BRIDGE_URL);
});